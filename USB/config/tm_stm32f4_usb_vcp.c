...
const uint8_t PackEnd[2] = {0X0D, 0X0A};

/* Private */
TM_USB_VCP_IRQ_Data TM_IRQ_Data;
TM_USB_VCP_IRQ TM_USB_VCP_IRQ_Status = TM_USB_VCP_IRQ_RESET;
...
void TM_USB_VCP_Reset_IRQ(void);
...
void TM_USB_VCP_Set_IRQ(uint8_t *data, uint16_t data_len)
{

	for (uint16_t i = 0; i < data_len; i++)
	{
		TM_IRQ_Data.Buff[TM_IRQ_Data.Data_Len] = data[i];

		if ((data[i] == PackEnd[0]) && (data[i + 1] == PackEnd[1]))
		{
			TM_USB_VCP_IRQ_Status = TM_USB_VCP_IRQ_SET;
			break;
		}
		else
			TM_IRQ_Data.Data_Len++;
	}
}

TM_USB_VCP_IRQ TM_USB_VCP_Get_IRQState(void)
{
	return TM_USB_VCP_IRQ_Status;
}

void TM_USB_VCP_Set_IRQPRC(void)
{
	TM_USB_VCP_IRQ_Status = TM_USB_VCP_IRQ_PRC;
}

void TM_USB_VCP_Reset_IRQ(void)
{
	TM_USB_VCP_IRQ_Status = TM_USB_VCP_IRQ_RESET;

	TM_IRQ_Data.Data_Len = 0;

	for (uint16_t i = 0; i < USB_VCP_RECEIVE_BUFFER_LENGTH; i++)
	{
		TM_IRQ_Data.Buff[i] = 0;
	}
}
